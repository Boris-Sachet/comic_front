...
class ServiceFile {
  ...
  static Future<File> regenFileData(File file) async {
    final response = await http.post(Uri.parse('http://${ServiceSettings.apiUrl}/file/${file.library.name}/${file.id}/regen'));
    if (response.statusCode == 200) {
      final Map<String, dynamic> json = jsonDecode(utf8.decode(response.bodyBytes));
      final File updatedFile = File.fromJson(json, file.library);
      return updatedFile;
    } else {
      throw Exception('Failed to regenerate file data');
    }
  }
}